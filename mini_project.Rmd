...
$$
du = 0 \Rightarrow u(t, x) =  cte = f(\xi) \leftarrow i.e. \text{ u ne depend que de } \xi 
$$
Soit alors 
...
$$
u(0, x) = u_0(x) = f(x) \Rightarrow f(x) = u_0(x)
$$
...
$$
u(t, x) = u_0(x-at), \ \forall x \in [0, L], \ \forall t > 0
$$
...
$$
\begin{aligned}
u_{j}^{n+1} &= u_{j}^{n} + \Delta t\frac{\partial u}{\partial t} + O(\Delta t^2)\\
\frac{\partial u}{\partial t} &=\frac{u_{j}^{n+1}-u_{j}^{n}}{\Delta t} + O(\Delta t)
\end{aligned}
$$
...
u_{j-1}^{n} &= u_{j}^{n} - \Delta x\frac{\partial u}{\partial x} + \frac{(\Delta x)^2}{2}\frac{\partial^2 u}{\partial x^2} + O((\Delta x)^3)\\
\frac{\partial u}{\partial x} &=\frac{u_{j+1}^{n}-u_{j-1}^{n}}{2\Delta x} + O((\Delta x)^2)\\
\end{aligned}
$$
